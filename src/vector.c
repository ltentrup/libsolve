...
void vector_print(vector* v) {
    printf("Vector (%ld,%ld) %s",v->count,v->size,"");
...
void vector_add_sorted(vector* v, void* value, int (*cmp)(void*, void*)) {
    ssize_t imin = 0, imax = v->count - 1;
    while (imax >= imin) {
        ssize_t imid = imin + ((imax - imin) / 2); // prevent overflow
        int res = cmp(v->data[imid], value);
        if (res == 0) {
            return;
        } else if (res < 0) {
            imin = imid + 1;
    ...
    vector_insert_at(v, imin, value);
    return;
}

size_t vector_find_sorted(vector* v, void* value, int (*cmp)(void*, void*)) {
    ssize_t imin = 0, imax = v->count - 1;
    while (imax >= imin) {
        ssize_t imid = imin + ((imax - imin) / 2); // prevent overflow
        int res = cmp(v->data[imid], value);
...
/**
 * Inserts value into sorted int vector, such that vector remains sorted.
 * Returns the index at which position the value was inserted.
 * If the value is already contained, no insertion is done but the index is returned.
 * Check count to determine if an insertion was done.
 */
size_t int_vector_add_sorted(int_vector* v, int value) {
    ssize_t imin = 0, imax = v->count - 1;
    while (imax >= imin) {
        ssize_t imid = imin + ((imax - imin) / 2); // prevent overflow
        if (v->data[imid] == value) {
    ...
    int_vector_insert_at(v, imin, value);
    return imin;
}

void int_vector_insert_at(int_vector* v, size_t index, int value) {
    if (v->size == v->count) {
        int_vector_increase(v);
    }
